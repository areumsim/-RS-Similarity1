...
Get.weighted.byGenre <- function( mat ){
  ...
  cat(seedNo, "byGenre", format(Sys.time(), usetz = TRUE), "\n")
  for( k in 1:genreCnt ){
    # cat(seedNo, "- [", k, "] byGenre", format(Sys.time(), usetz = TRUE), "\n")
    
    trgIdx <- movie_genre[movie_genre$genreNo==k, 1]
    
    # TODO : < 10  or < 0
    if( length(trgIdx) < minCnt ){
      genreWList[[k]] <- rep(0, siCnt)
    }else{
...
Get.weighted.byItem <- function( mat ){
  ...
  for( k in 1:ncol(mat) ){
    ...
    # initW <- genreWList[[ movie_genre[k,3] ]]
...
for( f in 31:40 ){
  ...
  # t[9] <- Get.mae.byGenre( testIdx , ratings, trainingRatingMat , genreWList )
  
  itemWList <- Get.weighted.byItem( trainingRatingMat ) # 9724 > 
  t[9] <- Get.mae.byItem( testIdx , ratings, trainingRatingMat , itemWList )  
  
  tmpSimList[[f]]  <- list(siMatList, totalWList , genreWList , itemWList)
  fold[[f]] <- t
