...
Reset.trainingset <- function( seedNo ){
  ...
  tmp <- list()
  cat(seedNo, "getSim\t", format(Sys.time(), usetz = TRUE), "\n")
...
Get.weighted.byItem <- function( mat ){
  ...
  cat(seedNo,"byItem",format(Sys.time(),usetz = TRUE),"\n")
  for( k in 1:ncol(mat) ){
...
#Predict.rating.bySim V2 - new version
Predict.rating.bySim <- function(userId,  itemId, mat, sim ){
  userDeviation <- mat[, itemId] 
  userDeviation[is.na(userDeviation)] <- 0
  
  #  sim(U,V) * ( rating of V - avg of V ) / abs sumSim only used ratings
  predictRating <- ( sim[1,] %*% userDeviation ) / (abs(sim[1,]) %*% ifelse(is.na(mat[, itemId]), 0, 1))
...
# 
# Predict.rating.bySim <- function(userId,  itemId, mat, sim ){
#   # ( sim(U,V) *  rating of V  ) / abs sumSim only used ratings
#   t <- mat[, itemId] 
#   t[is.na(t)] <- 0
#   predictRating <- ( sim[1,] %*% t ) / (abs(sim[1,]) %*% ifelse(is.na(mat[, itemId]), 0, 1))
#   
#   predictRating[predictRating>maxRating] <- maxRating
#   predictRating[predictRating<minRating] <- minRating
#   predictRating[is.nan(predictRating)] <- NA
...
k[1] <- Get.mae(testIdx, ratings, trainingRatingMat , siMatList[[1]])
k[2] <- Get.mae(testIdx, ratings, trainingRatingMat , siMatList[[2]])
k[3] <- Get.mae(testIdx, ratings, trainingRatingMat , siMatList[[3]])
k[4] <- Get.mae(testIdx, ratings, trainingRatingMat , siMatList[[4]])
k[5] <- Get.mae(testIdx, ratings, trainingRatingMat , siMatList[[5]])
k[6] <- Get.mae(testIdx, ratings, trainingRatingMat , siMatList[[6]])
k[7] <- Get.mae(testIdx, ratings, trainingRatingMat , siMatList[[7]])

totalWList <- Get.weighted( trainingRatingMat )
k[8] <- Get.mae( testIdx, ratings, trainingRatingMat, Get.sim.byWeighted(totalWList) )

itemWList <- Get.weighted.byItem( trainingRatingMat ) # 9724 > 
k[9] <- Get.mae.byItem( testIdx , ratings, trainingRatingMat , itemWList )  
