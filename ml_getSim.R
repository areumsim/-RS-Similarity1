...
Cal.all.default.similarity <- function( mat ){
  ...
  maxRating <- 5  # max(mat, na.rm = TRUE)
  minRating <- 0  # min(mat, na.rm = TRUE)
  
  siMat1 <- matrix(data = NA, ncol = userLen, nrow=userLen)   # Cos
  siMat2 <- matrix(data = NA, ncol = userLen, nrow=userLen)  # Pearson
  siMat3 <- matrix(data = NA, ncol = userLen, nrow=userLen)  # CPC
  siMat4 <- matrix(data = NA, ncol = userLen, nrow=userLen)  # MSD
  siMat5 <- matrix(data = NA, ncol = userLen, nrow=userLen)  # Jaccard
  siMat6 <- matrix(data = NA, ncol = userLen, nrow=userLen)  # aJaccard
  
  tmp <- list()
  for( i in 1:(userLen-1) ) {
    cat(i, "\t")
    print( Sys.time() )
  ...
  siMatList = list( siMat1, siMat2, siMat3, siMat4,  siMat5 , siMat6 )
  
  # save( siMatList , file="C:/Users/haji/Desktop/200518/DB/ml_small_siMatList.RData")
...
Cor.similarity <- function(a, b) {
  ...
  tmp2 = sqrt( sum((a - avgA) * (a - avgA), na.rm = T) )
  tmp3 = sqrt( sum((b - avgB) * (b - avgB), na.rm = T) )
...
Cpc.similarity <- function(a, b) {
  ...
  tmp2 = sqrt( sum((a - rMed) * (a - rMed), na.rm = T) )
  tmp3 = sqrt( sum((b - rMed) * (b - rMed), na.rm = T) )
